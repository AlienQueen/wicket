...
public class AtmosphereBehavior extends AbstractAjaxBehavior
	...
	public void onRequest() {
		...
		Broadcaster broadcaster = findEventBus().getBroadcaster();
		broadcaster.addAtmosphereResource(meteor.getAtmosphereResource());
		findEventBus().lookupAtmosphereResourceFactory().registerUuidForFindCandidate(resource);
	...
	public void renderHead(Component component, IHeaderResponse response) {
		...
		try {
			...
			if (AtmosphereTransport.WEBSOCKET == findEventBus().getParameters().getTransport()) {
				options.put("url", "/websocket/" + getCallbackUrl());
			} else {
				options.put("url", getCallbackUrl());
			}

			response.render(JavaScriptHeaderItem.forScript("$(function () { window.wicketAtmosphere(" + options.toString() + "); });", ""));
